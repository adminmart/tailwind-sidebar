  ...
  if (collapsible === "none") {
    return (
      <div
        data-slot="sidebar"
        className={cn(
          ` text-sidebar-foreground flex h-full w-(--sidebar-width) flex-col ${className}`,
          className
        )}
        {...props}
      >
  ...
  "peer/menu-button flex w-full  items-center gap-3 overflow-hidden rounded-md text-left text-sm outline-hidden ring-sidebar-ring transition-[width,height,padding] focus-visible:ring-2 disabled:pointer-events-none disabled:opacity-50 [&>span:last-child]:truncate [&>svg]:size-4 [&>svg]:shrink-0",
  {
    variants: {
      ...
      active: {
        ...
        false: "bg-transparent hover:bg-sidebar-accent hover:text-sidebar-accent-foreground dark:hover:text-sidebar-accent-foreground", // Hover styles only when not active
      },
    },
    defaultVariants: {
      variant: "default",
...
function SidebarMenuButton({
  ...
  const button = (
    <Comp
      ...
      className={cn("side_sub mb-0.5 ", sidebarMenuButtonVariants({ variant, size, active: isActive, }), className)}
      {...props}
    />
  )
